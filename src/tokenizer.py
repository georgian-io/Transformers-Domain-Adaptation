...
def _clean_args(tokenized, max_length, keep, cls_token, sep_token):
    ...
    if isinstance(keep, str) and keep not in STRATEGIES:
        ...
        if len(keep) != 2:
            raise ValueError('Expecting a `keep` list to be of length 2 but '
        ...
        if sum(keep) != max_length:
            raise ValueError('`keep` list should sum to 510 but got '
                             f'{sum(keep)} instead.')

    # Assign `cls_token` and `sep_token` if not provided
    DEFAULTS = {'cls': {str: '[CLS]', int: 101},
                'sep': {str: '[SEP]', int: 102}}
    ...
    if not(type(cls_token) == type(sep_token)
           and type(tokenized[0][0]) == type(sep_token)):
        raise ValueError('`tokenized`, `cls_token` and `sep_token` have to be '
                         'of the same type.')
...
def truncate(tokenized: Union[List[str],
                              List[List[str]],
                              List[int],
                              List[List[int]]],
             max_length: int = 510,
