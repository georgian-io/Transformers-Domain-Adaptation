...
def parse_args():
    ...
    parser.add_argument(
        "--src", type=Path, required=True, help="Input text file to be sharded"
    )
    parser.add_argument(
        "--dst", type=Path, required=True, help="Folder to store sharded files"
    )
    parser.add_argument(
        "-s",
        "--shard-size",
...
def main(args):
    ...
    with open(args.src) as f:
        for i, chunk in tqdm(
            enumerate(read_in_chunks(f, n_lines=args.shard_size)), desc="Sharding"
        ):
            shard = (args.dst / f"{args.src.stem}_{i:05d}").with_suffix(".txt")
