...
PCT=2
MOD="similar"
EXP_NAME="pubmed_${PCT}pct_${MOD}_all"

DATA_DIR="data/biology/corpus/subsets/"
RESULTS_DIR="results/$FINE_TUNE_DATASET/$EXP_NAME/domain-pre-trained"
CORPUS="pubmed_corpus_similar_all_linnaeus_train_2pct_count_linear_combo_stdscl.txt"
mkdir -p $DATA_DIR
mkdir -p $RESULTS_DIR
...
aws s3 cp "$BUCKET/domains/biology/corpus/subsets/bert_vocab/$CORPUS" \
    "data/biology/corpus/subsets/bert_vocab/$CORPUS"
